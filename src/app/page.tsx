...
import { Skull, MessageSquare, Send, Zap, MapPin, Calendar, Clock, AlertTriangle } from 'lucide-react';
import { verificarApodo, obtenerSesionActiva, confirmarAsistencia, publicarEnMuro, enviarMensajeChat } from './actions';

// EL DICCIONARIO DE MENSAJES CURSED
const mensajesPorUsuario: Record<string, string> = {
  ...
  china: '没错：我特意翻译了这个，就是为了让你能看懂。我邀请你来参加我的生日派对。',
  ...
  michael: 'maldito borracho bro, estas invitado a mi cum, si no vienes le mono ya no te va a besar',
};
...
export default function Home() {
  ...
  const handleMuroSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!mensajeMuro.trim() || !usuario) return;
    await publicarEnMuro(usuario.id, mensajeMuro);
    setPostsMuro([{ id: Date.now(), nombre: usuario.nombre, mensaje: mensajeMuro, creadoEn: new Date() }, ...postsMuro]);
    setMensajeMuro('');
  };
  ...
  // Obtenemos el mensaje personalizado o uno por defecto
    ...
    <main className="min-h-screen w-full bg-zinc-200 text-black font-mono selection:bg-black selection:text-white pb-20">
      <AnimatePresence mode="wait">
        ...
        {!usuario ? (
          ...
          >
            <div className="w-full max-w-md bg-zinc-100 p-8 border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] text-center">
              ...
              <form onSubmit={handleLogin} className="space-y-6">
                <div>
                  <input
                    ...
                    disabled={validando}
                    className="w-full px-4 py-4 bg-white border-4 border-black text-center font-black uppercase placeholder:text-zinc-400 focus:outline-none focus:bg-zinc-200 transition-none disabled:opacity-50"
                  />
                </div>

                {errorLanding && (
                  <div className="bg-black text-white p-2 font-bold uppercase text-xs animate-pulse">
                    {errorLanding}
                  </div>
                )}
                ...
                <button
                  type="submit"
                  disabled={validando}
                  className="w-full py-4 bg-white hover:bg-black hover:text-white border-4 border-black font-black uppercase text-lg shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] active:shadow-none active:translate-x-[4px] active:translate-y-[4px] transition-all cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
                >
          ...
          /* --- DASHBOARD BRUTALISTA --- */
          <motion.div
            key="dashboard"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="max-w-7xl mx-auto p-4 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-start"
          >
            {/* Header Cursed */}
            <div className="col-span-12 bg-white p-6 border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col md:flex-row items-center justify-between gap-4">
              <div>
              ...
              <div className="flex items-center gap-2 bg-black text-white px-4 py-2 border-2 border-black font-bold uppercase text-xs">
                invitación
              </div>
            </div>

            {/* COLUMNA IZQUIERDA: EL MURO */}
            <div className="lg:col-span-4 flex flex-col h-[550px] bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-6">
              <h3 className="text-xl font-black flex items-center gap-2 mb-4 border-b-4 border-black pb-2">
                no suban webadas p oño
              </h3>
              
              <form onSubmit={handleMuroSubmit} className="mb-4 flex gap-2">
                <input
                  type="text"
                ...
                />
                <button type="submit" className="p-2 bg-black text-white border-2 border-black hover:bg-zinc-800 cursor-pointer">
                  <Send size={18} />
                </button>
              </form>
              ...
              <div className="flex-1 overflow-y-auto space-y-4 pr-2">
                {postsMuro.length === 0 ? (
                  ...
                  postsMuro.map((post) => (
                    <div key={post.id} className="p-3 bg-zinc-100 border-2 border-black">
                      <p className="text-xs font-black uppercase border-b-2 border-black pb-1 mb-1">{post.nombre}</p>
                      <p className="text-sm font-bold">{post.mensaje}</p>
                    </div>
                  ))
                )}
              </div>
            ...
            <div className="lg:col-span-4 space-y-8">
              <div className="bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                <div className="w-full h-56 bg-zinc-200 border-b-4 border-black relative grayscale hover:grayscale-0 transition-all duration-500">
              ...
              <div className="bg-white p-4 border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                ...
                <div className="w-full h-40 border-4 border-black grayscale">
                  <iframe 
            ...
            <div className="lg:col-span-4 flex flex-col h-[550px] bg-zinc-900 text-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] p-6">
